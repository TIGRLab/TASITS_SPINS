...
```{r}
#Summary: The awareness of social inference test (TASIT) is a test that assesses social perception in individuals with social cognitive deficits. However, the long administration time of this test can be burdensome for participants and patients. TASIT-S is a short form of TASIT, and has been validated and used in acquired brain injury and dementia populations. However, this has not been done in schizophrenia spectrum disorders (SSDs), and doing so would allow its use in future clinical trials. Both forms of the test have 3 subtests. Each substest contains items whose scores are added together to come up with a total score for the subtest. In order to assess the validity of TASIT-S, TASIT-S subscores are correlated with TASIT subscores.
...
library(tidyverse)
#Inputting the necessary datasets: TASIT Item scores dataset 
spinsdata <- read.csv(file="/projects/aolorunsola/TASITS_SPINS/data/raw/spins_tasit_data_06-23-2023.csv",header=TRUE)

#Inputting necessary datasets: dataset that contains other relevant measures (other SPINS soc/cog data, as well as some other measures):
...
#remove MPRC site participants as they do not have the individual TASIT items captured (don't need these participants in the dataset). 
spins_df <- spins_df[spins_df$site!="mrc",]

#Refining "spinsdata" dataset to only include participants that are found in "spins_df" dataset (which contains some other measures that are not from TASIT). We only want participants that have both TASIT data as well as data for the other relevant measures because we will be making correlations using both.
...
#Removing participants that have missing TASIT data from "spinsdata" dataset. TASIT score missingness was determined in lines 337-343 of this script:
...
#Should then also remove these participants from the "spins_df" dataset so that each datset contains the same particpants. 
...
```{r}
#Code for TASIT-S Part 1 (creating TASIT-S:1 scores)
#rowSums (used to add scores from the multiple items within TASIT-S:1 so that we can get a total score for TASIT-S:1)
...
```{r}
#Code for TASIT-S Part 2 (creating TASIT-S:2 scores)
...
```{r}
# Code for TASIT-S Part 3 (creating TASIT-S:3 scores)
...
##correlation between TASIT-S:3 lies and TASIT:3 lies
...
#correlation between TASIT-S:3 sarcasm and TASIT:3 sarcasm
...
#Up to this point, all of the correlations have been done across all groups (that is, both patient and control scores together from TASIT-S are compared with patient and control scores together in the original TASIT). In the following code, correlations were made between TASIT-S and TASIT sub-scores between patients (SSDs) and controls separately.

#Making correlations between the scores of control group participants ONLY
#First, we need to filter the original dataset so that we only have the control group scores
...
print(control_data)
#A dataset with control scores only has now been made. The patient data (SSDs) has been removed
...
#correlation between TASIT-S:2 sincere and TASIT:2 sincere in controls
...
#correlation between TASIT-S:2 sarcasm and TASIT:2 sarcasm in controls
...
#TASIT-S:3 and TASIT:3 correlations in controls:
...
#First, we need to filter the original dataset so that we only have the patient group scores
...
#correlation between TASIT-S:2 sincere and TASIT:2 sincere in patients
...
#correlation between TASIT-S:2 sarcasm and TASIT:2 sarcasm in patients
...
#TASIT-S:3 and TASIT:3 correlations in patients:
...
```{r}
#Making a data-set that only contains TASIT:S scores (removing original TASIT scores)
#Run all of the code up to this point so that the total subscore variables for TASIT-S are existing in the dataset.
...
#Checking to see what specific scores these participants are missing. When doing this step, make sure the entire script has been run up to this point so that participants who were meant to be removed earlier on in the script are removed here as well. To check for specific missing scores in each participant, just replace the record ID in the code below. We can then remove any of these participants from the original spinsdata, and then rerun the entire script to make sure our correlations are updated without any missing values.
...
#If the code is up to date, the table for missingness that is generated in this chunk should be empty, as all participants with missing data have been removed.
```



```{r}
#Looking at distributions of variables in datasets - only TASIT-S variables and only TASIT variables
#TASIT-S Distributions:
#TASIT-S: need to melt data first, as ggplot function only uses long data format. So we subset the data (to select which columns/variables that we want to graph), and then we melt this data.
...
#TASIT Distributions:
#TASIT: Making distributions for variables that were used to make correlations with TASIT-S variables. Need to melt data first again.
...
#TASIT-S T-tests (by diagnostic group):
...
#merge by record_id = merge datasets by variable record_id (merge "spinsdata" dataset, which contains TASIT data of the participants, and "spins_df" dataset, which contains the social cognitive and other important measures of the participants, and most importantly the sex of the participants)
#Merging the datasets:
spins_total_data <- merge(spinsdata,spins_df,by="record_id")
print(spins_total_data)

#TASIT T-tests by sex:
...
#TASIT-S effect sizes (by diagnostic_group)
...
#Merging SPECIFIC spins_df variables with spinsdata dataset ("specific" meaning the variables that we will actually be using from this dataset). First need to create this datset that contains the measures we actually care about:
...
#Checking Normality
#Need to make a dataset that is not melted:
...
#Note: "demo_sex" variable was not included for the shapiro.test because it is a non numeric variable. It was also removed from the original distributions for the same reason. So, "demo_sex" variable was removed from "spins_df_specific" dataset and "spins_df_specific_shapirotest" dataset. It still remained in the original "spins_df" dataset though because this dataset was merged with "spinsdata" dataset to make T tests that were done based on sex (so this variable was required in this dataset). This way, the correlations for the soc cog data vs TASIT-S (later in script) data will not include any "demo_sex" correlations (because the dataset used for these correlations does not include "demo_sex" (which is the spins_df_specific dataset. This is the dataset that was merged with spinsdata to make correlations between TASIT-S and other relevant measures)).
```
...
#Creating tables:
#--------------------
#The first table that we will make contains the scores of some other relevant measures (that are not TASIT measures). First make a new spins_df dataset that contains all of the variables/measures that we would like to use for this first table (ex: age, education)
#Dataset that will be used for Table #1:
spins_df_table1 <- spins_df[, c("diagnostic_group", "demo_sex", "demo_age_study_entry", "demo_highest_grade_self")]
library(tableone)

#Making Table #1
table1 <- CreateTableOne(data = spins_df_table1[,c(1:4)], strata = c("diagnostic_group"))
print(table1, contDigits = 3)

#Making Table #2 (this is a table containing TASIT and TASIT-S scores)
spinsdata_table2 <- spins_total_data_specific[, c("diagnostic_group", "tasit_part1_total","tasit_part2_sinc", "tasit_part2_sarc", "tasit_part2_total", "tasit_part3_lies", "tasit_part3_sarc", "tasit_part3_total", "tasits_part1_total", "tasits_part2_sinc", "tasits_part2_sarc", "tasits_part2_total", "tasits_part3_lies", "tasits_part3_sarc", "tasits_part3_total")]
table2 <- CreateTableOne(data = spinsdata_table2[,c(1:15)], strata = c("diagnostic_group"))
print(table2, contDigits = 3)


#Creating Box plot graphs - case vs controls (for both TASIT and TASIT-S):
#--------------------
library(ggplot2)
...
#--------------------
library(corrplot)
#The following data frame only contains TASIT variables that we are interested in ("spinsdata" dataset contained many TASIT measures, a lot of which we are not interested in):
...
#Dataset that only contains TASIT-S variables:
...
#Note: named it tasits_data_only_1 because the original tasits_data_only variable (made earlier in script and was used to check for missingness) contained the redcap_event variable, which I don't want in the dataset
...
#Correlation matrix: TASIT-S vs other relevant measures
#--------------------
...
#Scatterplot:
#--------------------
#Make a dataset that only includes case data, as for this scatterplot I will be using the "sans_total" measurement, which was not completed in controls:
...
```
```{r}
#Checking for participant missingness in other relevant measures:
#We know that with all of the TASIT/TASIT-S values that we care about, there is no missing data (as we have already removed individuals with missing TASIT scores of interest). We therefore only need to check the "spins_df_specific_shapriotest" dataset to see who is missing data here. Basically, we are checking who has any missing data in these other relvant measures. (Note: "spins_df_specific_shapriotest" is the non melted complete dataset that contains all other measures of interest).

#Creating data frame that contains all participants with missing scores. Note - these participants only have missing scores from other measures. All of their TASIT/TASIT-S scores that we are interested in are present (not missing)
...
#Data frame containing individuals with missing data:
individuals_missing_data <- missingnesstable[!complete.cases(missingnesstable),]
print(individuals_missing_data)

#Any participant in this data frame has some missing measures. These missing measures are NOT TASIT variables, but are from the other soc cog, neuro cog, etc measures. 

#Create a missingness table for cases - include three other relevant measures (qls_total, bprs_factor_total, sans_total_sc):
missingnesstable_cases <- spins_df[, c("record_id","diagnostic_group", "scog_er40_total", "scog_rmet_total","scog_mean_ea", "scog_iri_perspective", "scog_iri_fantasy", "scog_iri_personal_distress", "scog_iri_empathic_concern", "bsfs_total", "np_domain_tscore_process_speed", "np_domain_tscore_att_vigilance", "np_domain_tscore_work_mem", "np_domain_tscore_verbal_learning", "np_domain_tscore_visual_learning", "np_domain_tscore_reasoning_ps", "np_domain_tscore_social_cog", "demo_age_study_entry", "qls_total", "bprs_factor_total", "sans_total_sc")]

missingnesstable_cases <- subset(missingnesstable_cases, diagnostic_group == "case")
cases_missing_data <- missingnesstable_cases[!complete.cases(missingnesstable_cases),]
print(cases_missing_data)

#Looking at cases and their scores for the qls_total, bprs_factor_total, and the sans_total_sc: Only two individuals had any missing scores (which were both from the qls_total measure). All other cases had no missing values for these three measures. IDs are SPN01_CMH_0002 and SPN01_ZHP_0129
```
